...
// ---------------------------------------------------------------------------
export interface GroveStreetRunOptions {
  demo?: boolean
}

export function useGroveStreetRun(
  canvasRef: Ref<HTMLCanvasElement | null>,
  options: GroveStreetRunOptions = {},
) {
  const isDemo = options.demo ?? false
  const state = ref<GameState>(isDemo ? "playing" : "idle")
  const score = ref(0)
  const highScore = ref(0)

  // Load high score from store on init (skip in demo no score tracking)
  if (!isDemo) {
  ...
  function update() {
    ...
    // Speed ramp
    const maxSpeed = isDemo ? 5 : MAX_SPEED
    gameSpeed = Math.min(maxSpeed, BASE_SPEED + frameCount * SPEED_INCREASE)
    ...
    // Demo autopilot scan ahead and jump over incoming obstacles
    if (isDemo) {
      const onGround = playerY + PLAYER_H >= GROUND_Y
      if (onGround) {
        const lookAhead = 90 + gameSpeed * 12
        const threat = obstacles.find(
          (o) => o.x > playerX && o.x < playerX + lookAhead && o.kind !== "sttam",
        )
        if (threat) {
          vy = JUMP_FORCE
        }
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
      ...
      // Collision (skip in dem the bot never dies)
      if (!isDemo) {
        const hitbox: Rect = {
          x: o.x + 2,
          y: o.y + 2,
  ...
  function mount() {
    ...
    // In demo mode the game is already in playing state just start the
    // render loop. In normal mode, the loop runs in idle to show the background.
    if (rafId === null) {
      rafId = requestAnimationFrame(loop)
    }
  }
